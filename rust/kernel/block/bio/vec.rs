...
pub(crate) const PAGE_SIZE: u32 = 1 << bindings::PAGE_SHIFT;
...
fn mp_bvec_iter_page_idx(bvec: *const bindings::bio_vec, iter: &bindings::bvec_iter) -> usize {
    (mp_bvec_iter_offset(bvec, iter) / PAGE_SIZE) as usize
}
...
fn bvec_iter_len(bvec: *const bindings::bio_vec, iter: &bindings::bvec_iter) -> u32 {
    mp_bvec_iter_len(bvec, iter).min(PAGE_SIZE - bvec_iter_offset(bvec, iter))
}
